...
public class ClubTypeDaoImpl extends BaseDaoImpl<ClubType> implements
		ClubTypeDao {

	@Override
	public int delete(Integer id) {
		String hql = "delete from ClubType where id = :id";
		return getCurrentSession().createQuery(hql).setInteger("id", id)
				.executeUpdate();
	}
	...
	public Boolean update(ClubType clubtype) {
		...
		int flag = getCurrentSession().createQuery(hql)
				.setString("name", clubtype.getName())
				.setInteger("id", clubtype.getId()).executeUpdate();
		if (flag != 0) {
			return true;
		} else {
			return false;
	...
	public List<ClubType> getTypeName(ClubType clubtype) {
		...
		List<ClubType> typename = getCurrentSession().createQuery(hql)
				.setString("name", clubtype.getName()).list();
		return typename;
	}

	public ClubType getByName(String name) {
		String hql = "from ClubType where name = :name ";
		ClubType clubtype = (ClubType) getCurrentSession().createQuery(hql)
